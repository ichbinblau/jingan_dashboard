...
widgetItemNewsWidget.prototype = 
	name : null
	current_page : null
	widget_type : null
	el : null
	...
	init : ( control_id )->
		...
		$.template_set "widgetItemNewsWidget" , self.tpl
	...
	showContent : (data , self) ->
		...
		console.log "#{self.el.id}:" , input
		$("._page#{self.current_page.id}").find("##{self.el.id}").html $.template("widgetItemNewsWidget",{ input : input })
