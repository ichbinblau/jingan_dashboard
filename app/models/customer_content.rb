# encoding: utf-8
class CustomerContent < CmsContent
  ...
  has_one :cms_info_customer,:foreign_key => "cms_content_id",:dependent => :destroy 
  attr_accessible :cms_info_customer_id, :cms_info_customer_attributes
  accepts_nested_attributes_for :cms_info_customer, :allow_destroy => true

  # has_many :cms_info_plans, :inverse_of => :customer_content
  ...
  protected
    def setup_beforeinfo
      # 添加表关联
        customerid=self.cms_info_customer.id 
        user_info_id = self.cms_info_customer.user_info_id
        self.cms_content_info_id = customerid
