  ...
  private
    ...
    private
      ...
      def handle_click_event
        ...
        # domain = "c873f95.ngrok.com"
        projectnum , action = @keyword.split "|"
        if action == "CLICK_TS1" ||  action == "CLICK_TS2" ||  action == "CLICK_TS3" ||  action == "CLICK_TSFAV"
          sign = action[8,6]

          projectinfo = ProjectInfo.where( :project_num => projectnum ).first
          project_cfg = JSON.parse projectinfo.app_config
          # Rails.logger.info "======="+@weixin_message.FromUserName.to_json
          # Rails.logger.info "======="+sign.to_json
          title = project_cfg["train_sorts"][sign]["name"]
          desc = project_cfg["train_sorts"][sign]["des"]
          article = generate_article( title ,  desc , "", "http://#{domain}/#{projectnum}/yixue/fake_login?weixin_id=#{@weixin_message.FromUserName.to_s}&sign=#{sign}")
          return reply_news_message([article])
        end
